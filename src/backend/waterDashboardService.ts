// NOTE: This file contains admin dashboard flow helpers. Use
// src/backend/waterAdminDashboardService.ts for Water admin dashboard-only logic.
// Regular user-facing water services should use src/backend/waterComplaintService.ts.
...
export async function fetchWaterDashboardStats(): Promise<WaterDashboardStat[]> {
  ...
  const pendingComplaints = complaints.filter((item) => item.status === "submitted" || item.status === "assigned").length;
  ...
  const stats: WaterDashboardStat[] = [
    ...
    },
    {
      ...
      trendDirection: "up",
      path: "/water/bill-payment?propertyId=12345678",
    },
    {
...
export async function fetchWaterDetailItems(category: WaterDashboardCategory): Promise<WaterDashboardDetailItem[]> {
  ...
  const details: WaterDashboardDetailItem[] = (
    category === "applications"
      ...
      : category === "pendingComplaints"
      ? complaints
          .filter((complaint) => complaint.status === "submitted" || complaint.status === "assigned")
          .map((complaint) => ({
            id: complaint.id,
...
export async function fetchWaterActionItems(
  ...
  const overdueItems = payments
    ...
    .map((item) => ({
      ...
      modulePath: `/water/bill-payment?propertyId=${item.propertyId}`,
      date: item.dueDate,
...
export async function fetchWaterNotifications(): Promise<WaterNotification[]> {
  ...
  const notifications: WaterNotification[] = [
    ...
    },
    {
      ...
      modulePath: "/water/bill-payment?propertyId=12345678",
    },
  ];
