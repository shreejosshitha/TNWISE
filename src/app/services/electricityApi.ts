...
const BASE_URL = '/api/electricity';
...
async createComplaint(complaintData: any, token: string): Promise<ApiResponse<any>> {
  ...
  async getAdminStats(): Promise<ApiResponse<any>> {
    const response = await fetch('/api/admin/electricity/stats');
    return response.json();
  },

  async getAdminApplications(limit = 10): Promise<ApiResponse<any>> {
    const response = await fetch(`/api/admin/electricity/applications?limit=${limit}`);
    return response.json();
  },

  async getAdminComplaints(limit = 10): Promise<ApiResponse<any>> {
    const response = await fetch(`/api/admin/electricity/complaints?limit=${limit}`);
    return response.json();
  },
  ...
  async updateComplaintStatus(id: string, data: { status: string; assignedTo?: string; notes?: string }, token: string): Promise<ApiResponse<any>> {
    ...
    const response = await fetch(`/api/admin/electricity/complaints/${id}/status`, {
      method: 'PUT',
  ...
  async approveApplication(id: string, decision: any, token: string): Promise<ApiResponse<any>> {
    const headers: Record<string, string> = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
    const response = await fetch(`/api/admin/electricity/applications/${id}/approve`, {
      method: 'PUT',
      headers: headers as HeadersInit,
  ...
  async getAdminBills(limit = 10, status?: string): Promise<ApiResponse<any>> {
    ...
    const response = await fetch(`/api/admin/electricity/bills?${params.toString()}`);
    return response.json();
  },

  async searchAdmin(q: string): Promise<ApiResponse<any>> {
    const response = await fetch(`/api/admin/electricity/search?q=${encodeURIComponent(q)}`);
    return response.json();
  },
  ...
  async getDetailedAdminStats(from?: string, to?: string): Promise<ApiResponse<any>> {
    ...
    const url = `/api/admin/electricity/stats-detailed${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  async getApplicationById(id: string, token?: string): Promise<ApiResponse<any>> {
    ...
    if (token) headers.Authorization = `Bearer ${token}`;
    const response = await fetch(`/api/electricity/tracker/${id}`, { headers });
    return response.json();
  },
  ...
  async updateApplicationStage(id: string, stage: string, notes?: string, token: string): Promise<ApiResponse<any>> {
    ...
    const response = await fetch(`/api/admin/electricity/applications/${id}/stage`, {
      method: 'POST',
      headers,
